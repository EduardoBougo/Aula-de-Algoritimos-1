...
Data..: 23/06/2025
Enunciado:
    (desafio 2): Crie um programa em Linguagem C para cadastrar bandas de música. O programa deve ter um registro contendo os seguintes dados da banda: nome da banda (string (50)), gênero musical (string (30)) e número de integrantes (inteiro). Usando funções, implemente as seguintes funcionalidades:

    1. cadastrar até o limite de 20 bandas OU pare antes se o usuário assim decidir.
    2. mostrar uma listagem (com todas as informações) das bandas. Apresentar os dados de uma forma tabular, alinhados e somente das bandas cadastradas;
    3. mostrar somente a(s) banda(s) que tocam um determinado gênero musical (o usuário vai digitar o gênero escolhido, textualmente);
    4. mostrar qual a quantidade média de integrantes das bandas (média aritmética simples).
*/
...
struct Data{
    ...
    int num_integrantes;
} dt[N_BANDAS]; // Declaração do vetor com os dados
